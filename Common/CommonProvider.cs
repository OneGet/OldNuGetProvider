        ...
        protected void GetPackageDependenciesImpl(string fastPackageReference, CommonRequest request) {
            ...
            foreach (var depSet in pkgRef.Package.DependencySets) {
                foreach (var dep in depSet.Dependencies) {
                    ...
                    if (depRefs.Length == 0) {
                        request.Error(ErrorCategory.InvalidResult, pkgRef.GetCanonicalId(request), Constants.Messages.DependencyResolutionError, request.GetCanonicalPackageId(PackageProviderName, dep.Id, ((object)dep.VersionSpec ?? "").ToString()));
                    }
